...
void update_infos()
{
        ...
        city->city_mean_contamination = city_tiles_contamination_sum
                / (CITY_HEIGHT * CITY_WIDTH);
}
                        ...
                        for (i = row - 1; i <= row + 1; i++) {
                                for (j = col - 1; j <= col + 1; j++) {
                                        ...
                                        if (city->map[j][i].type != WASTELAND
                                            ...
                                            <= city->map[j][i].contamination) {
                                                continue;
                                        }
                    
                                        if (generate_random_percentage()
                                            > PROBABILITY_WASTELAND_CONTAMINATION) {
                                                continue;
                                        }
