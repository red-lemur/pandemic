...
void send_news(status_t *status)
{
        ...
        if (status->type == JOURNALIST) {
                ...
                sprintf(buffer, "%ud %.3lf %s", status->id, status->contamination,
                        status->name);
        ...
        for (;;) {
                if (citizen_can_leave_tile(status)) {
                        ...
                        if (city->map[status->x][status->y].type == HOSPITAL
                            && !status->is_sick
                            && (status->type == SIMPLE_CITIZEN
                                || status->type == JOURNALIST)) {
                                pthread_mutex_lock(&mutex);
                                status->days_spent_in_hospital_healthy++;
                                pthread_mutex_unlock(&mutex);
                        }
                        
                        if (status->days_spent_in_hospital_healthy
                            == MAX_DAYS_IN_HOSPITAL_HEALTHY) {
                                pthread_mutex_lock(&mutex);
                                status->must_leave = 1;
                                status->days_spent_in_hospital_healthy = 0;
                                pthread_mutex_unlock(&mutex);
...
void sprayer_decontamination(status_t *status)
{
        ...
        do {
                most_contaminated
                        = get_most_contaminated_citizen_of_tile(status,
                                                                &(city->map[status->x][status->y]));
                ...
                if (most_contaminated == NULL) {
                        ...
                        if (city->map[status->x][status->y].contamination
                            < MAX_SPRAYER_DECONTAMINATION) {
                                decontamine_tile_with_sprayer(status,
                                                              &(city->map[status->x][status->y]),
...
status_t *get_most_contaminated_citizen_of_tile(status_t *fireman_status, tile_t *tile)
{
        ...
        for (i = 0; i < CITIZENS_NB; i++) {
                ...
                } else if (most_contaminated->contamination
...
void risk_of_death(status_t *status)
{
        ...
        if (generate_random_percentage() <= death_reduction
            * (status->sickness_duration - DAYS_NB_DISEASE_DEADLY + 1)
            * PROB_OF_DEATH_BY_DAY) {
                pthread_mutex_lock(&mutex);
                status->type = DEAD;
                pthread_mutex_unlock(&mutex);
        }
...
int main(void)
{
        ...
        do {
                fifo_from_epidemic_sim = open(FIFO_EPIDEMIC_SIM_TO_CITIZEN_MANAGER_URL,
                                              O_RDONLY);
        ...
        city = mmap(NULL, sizeof(city_t), PROT_READ | PROT_WRITE, MAP_SHARED,
                    shared_memory, 0);
